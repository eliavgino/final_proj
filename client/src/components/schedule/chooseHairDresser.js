...
const ChooseHairCut = () => {
   ...
   function handleClick(selectedBarber) {
    ...
    setButtonState(selectedBarber ? selectedBarber.barber_Name : barbers[randomIndex].barber_Name);
  }
          ...
          <div className='container'>
              ...
              {barbers.map(barber=>  <button
                  ...
                  <div><button 
                                  ...
                                  className={`chooseHairDresser ${buttonState === barbers[0].barber_Name ? 'clicked-button' : ''}`}>
                                  I dont care
                                </button></div>

          </div>
          <div style={{justifyContent:'center',display:'flex',flexDirection:'row',alignItems:'center'} }>
              <button id="nextBtn"   onClick={()=>{setPageState("chooseDate")}}>Back</button>
              <button id="nextBtn" disabled={chooseBarber ? false : true}  onClick={()=>{setPageState("confirm")}}>Next</button>
          </div>
      </div>
  </div>
  </div>
